...
def send_to_data_base(
    ...
    employee_data = get_data(
        ...
        f"?name={table_lines['employee']}"
    )

    if not employee_data:
        alert = (
            "⚠️ Can't find employee, issue don't save "
            "to database, please check your name and try again."
        )
    ...
    robot_data = get_data(
        ...
        f"?robot_number={int(table_lines['robot'])}"
        f"&warehouse=GLP-C"
        f"&limit=1"
    )
