...
| Описание причины при `/offline` / `/online` | бот удаляет, приняв его |
...
Отключается переменной `DELETE_USER_MESSAGES=false`. В личных чатах бот не
имеет права удалять чужие сообщения — там шаг просто пропускается.
...
Эндпоинт `/p/<файл>` отдаёт 302 на подписанный URL Supabase (подпись
кэшируется на час), поэтому bucket остаётся приватным. Домен задаётся
`PHOTO_LINK_BASE`, а `PUBLIC_PHOTO_URLS=true` переключает на публичные ссылки
Supabase (тогда bucket становится публичным) — они не зависят от работы бота.
